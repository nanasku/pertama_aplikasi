...
// GET semua data harga beli
router.get('/', (req, res) => {
  const query = 'SELECT * FROM harga_beli ORDER BY nama_kayu';
  
  db.query(query, (err, results) => {
    if (err) {
      console.error('Error fetching harga beli:', err);
...
// GET data harga beli by ID
router.get('/:id', (req, res) => {
  const { id } = req.params;
  const query = 'SELECT * FROM harga_beli WHERE id = ?';
...
// POST tambah data harga beli
router.post('/', (req, res) => {
  const { nama_kayu, prices } = req.body;
  
  const query = `
    INSERT INTO harga_beli 
  ...
  db.query(query, values, (err, results) => {
    if (err) {
      console.error('Error creating harga beli:', err);
      return res.status(500).json({ error: 'Database error' });
    }
...
router.put('/:id', (req, res) => {
  ...
  db.query(query, values, (err, results) => {
    if (err) {
      console.error('Error updating harga beli:', err);
      return res.status(500).json({ error: 'Database error' });
    }
...
// DELETE data harga beli
router.delete('/:id', (req, res) => {
  const { id } = req.params;
  const query = 'DELETE FROM harga_beli WHERE id = ?';
  
  db.query(query, [id], (err, results) => {
    if (err) {
      console.error('Error deleting harga beli:', err);
      return res.status(500).json({ error: 'Database error' });
    }
